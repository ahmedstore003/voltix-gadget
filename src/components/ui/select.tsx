...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex h-11 w-full items-center justify-between gap-2 border border-border bg-card px-3 py-2.5 text-sm text-foreground shadow-sm',
      'focus:outline-none focus:border-muted-foreground transition-colors',
      'disabled:cursor-not-allowed disabled:opacity-50',
      '[&>span]:line-clamp-1 text-start',
      className
    )}
    {...props}
  >
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      'relative flex w-full cursor-pointer select-none items-center rounded-sm py-2.5 ps-8 pe-3 text-sm outline-none',
